...
const categories = {
  '상의': ['니트', '후드', '맨투맨', '셔츠/블라우스', '긴소매 티셔츠', '반소매 티셔츠', '민소매 티셔츠', '카라 티셔츠', '베스트'],
  '바지': ['데님 팬츠', '슬랙스', '트레이닝/조거팬츠', '숏팬츠', '코튼 팬츠', '레깅스', '와이드 팬츠'],
  '아우터': ['후드 집업', '바람막이', '코트', '롱패딩', '숏패딩', '패딩 베스트', '블루종/MA-1', '라이더 자켓', '무스탕', '트러커 자켓', '블레이저', '가디건', '뽀글이 후리스', '사파리 자켓'],
...
const CategoryTabs = () => {
  const [currentCategory, setCurrentCategory] = useState(Object.keys(categories)[0]);
    ...
    <div className='CategoryTabs'>
      ...
      <div className="category-buttons">
        {categories[currentCategory].map((subcategory) => (
          <CategoryButton key={subcategory} label={subcategory} onClick={() => console.log(subcategory)} />
        ))}
      </div>
